...
void Sendnotification(const char* titleText, const char* descText) {
    ...
    for (size_t i = 0, j = strlen(titleText); i < j; i++) utf8_to_utf16(title[i], titleText[i], strlen(titleText[i]));
    for (size_t i = 0, j = strlen(descText); i < j; i++) utf8_to_utf16(message[i], descText[i], strlen(descText[i]));
